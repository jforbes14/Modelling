...
fit_ss_16_noint <- lm(Perc_LNP ~ ., 
                data = data_mod %>% 
                  filter(year == "2016") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))
...
int16 <- interact_F(update(fit_ss_16_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2016")
int13 <- interact_F(update(fit_ss_13_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2013") 
int10 <- interact_F(update(fit_ss_10_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2010") 
int07 <- interact_F(update(fit_ss_07_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2007") 
int04 <- interact_F(update(fit_ss_04_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2004") 
int01 <- interact_F(update(fit_ss_01_noint, . ~ . + Educ:MedianAge + Extractive:Educ)) %>% mutate(year = "2001") 
...
## Adding third
int16 <- interact_F(update(fit_ss_16_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2016")
int13 <- interact_F(update(fit_ss_13_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2013") 
int10 <- interact_F(update(fit_ss_10_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2010") 
int07 <- interact_F(update(fit_ss_07_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2007") 
int04 <- interact_F(update(fit_ss_04_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2004") 
int01 <- interact_F(update(fit_ss_01_noint, . ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House)) %>% mutate(year = "2001") 
...
We now iterate by adding the interaction that is significant in most years. First we choose MedianAge:Educ, then Extractive:Educ and OneParent_House:Educ. Now we see that no interaction is significant in at least half of the elections (at the 1% level).


```{r}
fit_ss_16 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2016") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))

fit_ss_13 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2013") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))

fit_ss_10 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2010") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))

fit_ss_07 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2007") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))

fit_ss_04 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2004") %>% 
                  select(Perc_LNP,  (superset_adj$varname %>% as.character)))

fit_ss_01 <- lm(Perc_LNP ~ . + Educ:MedianAge + Extractive:Educ + Educ:OneParent_House, 
                data = data_mod %>% 
                  filter(year == "2001") %>% 
                  select(Perc_LNP, (superset_adj$varname %>% as.character)))
```
...
```{r}
aug_all %>% 
  left_join(data_mod %>% select(SocialServ, year, Election_Division), 
            by = c("SocialServ", "year")) %>% 
...
```{r}
vars_not_in <- names(data_mod)[which(!names(data_mod) %in% superset_adj$varname)]
...
aug_all %>% bind_cols(data_mod %>% filter(!is.na(Perc_LNP))) %>% select(-ends_with("1")) %>%  filter(.cooksd > 0.1)
```

Across the six elections, only Sydney in 2007 has moderately high leverage (cook distance of 0.140). This is due to its strong support for Labor (69.5% of TPP vote), whilst having high RentLoan, Educ, CurrentlyStudying and Incomes. Sydney also has moderate leverage in 2016 (cook distance of 0.106), as did Kalgoorlie (2004, 0.126) Hunter (2010, 0.112) and Durack (2016, 0.101). Therefore, it does not appear that there are any outliers that should be removed from the model. 
