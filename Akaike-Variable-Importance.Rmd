...
```{r}
std_coef_2016 <- fiveway_2016 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))

std_coef_2013 <- fiveway_2013 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))

std_coef_2010 <- fiveway_2010 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))

std_coef_2007 <- fiveway_2007 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))

std_coef_2004 <- fiveway_2004 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))

std_coef_2001 <- fiveway_2001 %>% unnest %>% group_by(vars) %>% summarise(avg_coef = mean(coef))
...
save(std_coef_all, file = "./Clean-Data/std_coef_all.rda")
